...
NEW_REVIEWS = 50
TOK_K = 5
CACHE_FILE = "project/zomato/data/review_embeddings.parquet"
...
def read_reviews_from_snowflake():
    ...
    query = f"""
        ...
        FROM zomato.silver.silver_reviews
        TABLESAMPLE ({NEW_REVIEWS} ROWS)
    """
    df = conn.cursor().execute(query).fetchall_arrow().to_pandas()
    conn.close()
